...
class ExportHelper
  ...
  def to_csv(survey_answers, survey_id)
    ...
    journal_ids.each do |j, survey_ids|
      csv_answers = CsvSurveyAnswer.by_journal_and_survey(j, survey_ids).map {|ca| ca.answer.chomp.gsub!(/^\"|\n"$/, ""); ca} #.group_by { |c| c.survey_id }
  ...
  def survey_headers(survey_id)
    # s_headers = cache_fetch("survey_headers_#{survey_ids.join('-')}", :expires_in => 15.minutes) do
      ...
      # ss.inject(Dictionary.new) { |hash, s| hash[s.id] = s.variable; hash }.order_by
    # end
  end
  ...
  def journal_entries(entries)
    t = Time.now
    survey_id = entries.map {|e| e.survey_id}.uniq.first
    s_headers = survey_headers(survey_id)
    headers = journal_csv_header.merge(s_headers)
    e = Time.now
