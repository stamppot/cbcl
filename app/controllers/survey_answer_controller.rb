...
class SurveyAnswerController < ApplicationController
  ...
  def create  # was: answer
    ...
    redirect_to :controller => :journal, :action => :show, :id => @journal_entry.journal if flash[:error]
    ...
    if survey_answer.save # and not @journal_entry.nil?
      ...
      if current_user and current_user.has_access? :all_users
        ...
        redirect_to :action => :finish, :id => @journal_entry.login_user
      end
    else
      flash[:notice] = "Fejl! Dit svar blev ikke gemt."
      redirect_to :action => :show, :id => @journal_entry
    end
  rescue RuntimeError
  ...
  def update  # was: save_changed_answer
    ...
    if survey_answer.save
      redirect_to :controller => :journal, :action => :show, :id => @journal_entry.journal_id
    else  # not answered
