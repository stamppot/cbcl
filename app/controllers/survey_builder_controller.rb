...
class SurveyBuilderController < ApplicationController
  ...
  def show
    redirect_to :controller => :survey, :action => :show
  end

  def show_fast
    redirect_to :controller => :survey, :action => :show_fast
  end
  ...
  def update
    ...
    if @survey.update_attributes(params[:survey])
      ...
      redirect_to :controller => :survey, :action => :list #, :id => @survey
    else
      render :action => :edit
    end
  end
  ...
  def save_question
    ...
    q_number = params[:question][:number]
    cells = sanitize_params(params)
    # render_text cells.inspect
    #puts "Transformed: " << cells.inspect
    ...
    # render_text "Sanitized Cells: #{cells.inspect}"
    ...
    cells.each do |cell, attributes|
      ...
      # render_text @q_cell.inspect
      @question.question_cells << @q_cell
      # render_text @question.question_cells.map { |i| i.inspect }.inspect
      @q_cell.preferences = ""  # preferences should be set somewhere. Should be included in sanitizied_params
      @q_cell.save
    end
    ...
    redirect_to :action => "add_question", :id => @survey
  end
  ...
  def change_form
    ...
    render :update do |page|
      ...
      change_options << link_to_function("skift", remote_function(
          :url => { :action => :change_back_form, :id => args[:cell] }),
          html_options) # << "</div>"
...
#      page.select("tr.row").each do |element, index|
  ...
  def superadmin_access
    if !current_user.nil? && current_user.has_access?(:admin)
      return true
