...
class CenterController < ApplicationController # < ActiveRbac::ComponentController
  ...
  def index
    redirect_to :controller => 'center', :action  => 'list'
  end
  ...
  def show
    ...
    redirect_to :controller => 'team', :action => :show, :id => @group if @group.instance_of?(Team)
  ...
  def new
    ...
    if request.post?
      ...
      # @group.center_info.update_attributes(params[:group].delete(:center_info))
      @group.update_attributes(params[:group])

      # assign properties to group
      if @group.save
        flash[:notice] = 'Centeret er blevet ' + (params[:id].nil? ? 'oprettet.' : 'opdateret')
        params[:id].nil? ? redirect_to(:action => :show, :id => @group) : redirect_to(:action => :list)
      else
        render :action => :new
      end
    end
  ...
  def pay_subscriptions
    ...
    if request.post?
      ...
      redirect_to :action => :show, :id => @group and return if @group.save
    end
