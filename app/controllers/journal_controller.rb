...
class JournalController < ApplicationController # < ActiveRbac::ComponentController
  ...
  def remove_survey
    ...
    if request.post?
      ...
      entries = JournalEntry.find(entries, :include => [:user, :survey_answer])

      entries.each do |entry| 
        entry.kill!
      end  # deletes user and survey_answer too
