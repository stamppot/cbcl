class CsvAnswer < ActiveRecord::Base
  ...
  # named_scope :for_survey, lambda { |survey_id| { :conditions => ["csv_answers.survey_id = ?", survey_id] } }
  # named_scope :for_center, lambda { |center_id| { :conditions => ["csv_answers.center_id = ?", center_id] } }
  # named_scope :for_team, lambda { |team_id| { :conditions => ["csv_answers.team_id = ?", team_id] } }
  ...
  #   query = CsvAnswer.for_survey(o[:survey][:id]).
