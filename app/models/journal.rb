...
class Journal < Group
  ...
  named_scope :with_login_user, :include => {:journal_entries => :login_user}
  named_scope :with_entries, :include => :journal_entries
  ...
  def info
    ...
    c["dagsdato"] = self.created_at.to_s(:db) #.strftime("%d-%b-%Y")
    c["pfoedt"] = self.birthdate.to_s(:db) #strftime("%d-%b-%Y")  # TODO: translate month to danish
    c
  end
